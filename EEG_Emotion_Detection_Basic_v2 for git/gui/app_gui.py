...
def preprocess_raw_eeg(data):
    return data  # Add any filtering or cleaning here if needed

# Clean and convert raw CSV EEG
def clean_and_convert_raw_csv(filepath):
    df = pd.read_csv(filepath)
    df = df.dropna(axis=1, how='all')  # Drop columns that are fully empty
    if df.shape[1] < 14:
        raise ValueError("CSV EEG file must contain at least 14 channels")
    eeg_data = df.iloc[:, :14].T.values
    if eeg_data.shape[1] < 128:
        raise ValueError("CSV EEG file must contain at least 128 time samples")
    return eeg_data
...
metrics_label = tk.Label(frame_info, text="Status: Waiting for file upload...", font=("Helvetica", 15), bg="#111111", fg="lightgreen")
...
def process_eeg_file(filepath):
    try:
        ...
        if ext == 'csv':
            eeg_data = clean_and_convert_raw_csv(filepath)
        elif ext == 'edf':
...
upload_btn = tk.Button(frame_info, text="📂 Upload EEG File", font=("Helvetica", 16), bg="#1e88e5", fg="white", command=lambda: process_eeg_file(filedialog.askopenfilename(filetypes=[("EEG Files", "*.csv *.edf *.mat")])) )
